...
export const AddTeamModal = ({onClose, onSuccess }) => {
    const {addTeam, teams} = useData()
    ...
    const validate = () => {
      ...
      teams.some(
        t => t.code.toLowerCase() === formData.code.trim().toLowerCase()
      )
    ) {
    ...
    return(
        <div className='fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4'>
            <div className='bg-white rounded-xl shadow-2x1 max-w-md w-full'>
                {/* Header */ }
                ...
                <div className='flex items-center justify-end gap-3 p-6 border-t border-gray-200'>
                    ...
                    type='button'
                    onClick={onClose}
                    className='px-4 py-2 border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-white transition-colors'
                    >
                    Cancel
                    </button>
                    <button
                    onClick={handleSubmit}
                    className='px-4 py-2 bg-[#C3002F] text-white rounded-lg text-sm font-medium hover:bg-[#A00024] transition-colors'
                    >
                    Add Team
                    </button>
                </div>
